// Backend copy of src/features/processing/courseMatcher.ts's scoring algorithm.
// Duplicated rather than imported: that file is frontend-owned per
// SHARED_CONTRACTS.md and imports via the '@/types' alias, which this Deno
// runtime has no import map for. Keep the scoring formula (thresholds,
// weights) identical between the two copies if either changes.

export type CourseRow = { id: string; code: string; name: string; professor: string };
export type CaptureAnalysisSignals = { courseSignals: string[]; topicSignals: string[] };
export type CourseMatchResult = { course: CourseRow | null; confidence: number; automatic: boolean; explanation: string };
...
function overlapScore(signals: string[], course: CourseRow): number {
...
function scoreCourse(analysis: CaptureAnalysisSignals, course: CourseRow) {
...
export function matchEnrolledCourse(analysis: CaptureAnalysisSignals, courses: CourseRow[]): CourseMatchResult {
